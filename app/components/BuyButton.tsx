...
export default function BuyButton({ title, price }: BuyButtonProps) {
  ...
  const roundedPrice = roundUpToNearestTenDollars(price);

  const handlePurchase = () => {
    // Redirect to the checkout page, passing product details via query parameters
    router.push(`/checkout?product=${encodeURIComponent(title)}&price=${roundedPrice}`);
  };
  ...
  return (
    ...
    >
      <span className="relative z-10">Purchase for {formatRoundedDollars(price)}</span>
